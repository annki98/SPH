...
public class Manager : MonoBehaviour
{
    ...
    private void Initialize()
    {
        ...
        colliders = new ParticleCollider[Amount];
        for (int i = 0; i < Amount; i++)
        {
            ...
            ParticleCollider currentCollider = currentGo.AddComponent<ParticleCollider>();
            colliders[i] = currentCollider;
            colliders[i].Position = currentParticle.Position;
        }
    }
    ...
    private void CalculateCollisions()
    {
        if(WallsUp)
        {
            for (int i = 0; i < particles.Length; i++)
            {
                for (int j = 0; j < Walls.Count; j++)
                {
                    ...
                    if (Collision(Walls[j], particles[i].Position, Radius, out penetrationNormal,
        ...
        for (int i = 0; i < particles.Length; i++)
        {
            colliders[i].Position = particles[i].Position;
        }
    ...
    private bool Collision(GameObject wall, Vector3 position, float radius, out Vector3 penetrationNormal, out Vector3 penetrationPosition, out float penetrationLength)
    {
        Vector3 colliderProjection = wall.transform.position - position;

        penetrationNormal = Vector3.Cross(wall.transform.forward, wall.transform.up);
        penetrationLength = Mathf.Abs(Vector3.Dot(colliderProjection, penetrationNormal)) - (radius / 2.0f);
        penetrationPosition = wall.transform.position - colliderProjection;

        return penetrationLength < 0.0f
            && Mathf.Abs(Vector3.Dot(colliderProjection, wall.transform.forward)) < wall.GetComponent<MeshCollider>().bounds.extents.x
            && Mathf.Abs(Vector3.Dot(colliderProjection, wall.transform.up)) < wall.GetComponent<MeshCollider>().bounds.extents.y;
    }
